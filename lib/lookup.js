...
function resolve(context, next) {
  ...
  if (meta) {
    ...
    if (rep) {
      ...
      if (rep.script && process.platform != 'win32') {
        context.emit('info','lookup-script',rep.script);
        context.module.script = rep.script;
      } else if (rep.scriptWin && process.platform === 'win32') {
        context.emit('info','lookup-script',rep.scriptWin);
        context.module.script = rep.scriptWin;
      }
